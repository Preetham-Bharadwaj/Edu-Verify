...
export async function fetchReportApplications(reqUser) {
    ...
    let records = apps.map((app) => {
        ...
        return {
            ...
            category: profile.institution_type || detailsExt.institution_type || detail.institution_type || 'School',
            father_occupation: app.father_occupation || detail.father_occupation || detailsExt.father_occupation || '',
            father_income: app.father_annual_income != null ? Number(app.father_annual_income) : (detail.father_annual_income != null ? Number(detail.father_annual_income) : ''),
            mother_occupation: app.mother_occupation || detail.mother_occupation || detailsExt.mother_occupation || '',
            mother_income: app.mother_annual_income != null ? Number(app.mother_annual_income) : (detail.mother_annual_income != null ? Number(detail.mother_annual_income) : ''),
            combined_family_income: app.combined_family_income != null ? Number(app.combined_family_income) : getAnnualIncome(fatherTax, motherTax),
            income_verification_status: app.income_verification_status || '',
            application_year: app.application_year || new Date(app.created_at).getFullYear() || '',
...
function drawApplicationTable(doc, rows) {
    const columns = [
        { label: 'Application ID', width: 65, key: 'application_id' },
        { label: 'Student Name', width: 65, key: 'student_name' },
        { label: 'Student ID', width: 50, key: 'student_id' },
        { label: 'Institution', width: 65, key: 'institution' },
        { label: 'Course', width: 40, key: 'course' },
        { label: 'Category', width: 40, key: 'category' },
        { label: 'Father Occ.', width: 50, key: 'father_occupation' },
        { label: 'Father Inc.', width: 45, key: 'father_income' },
        { label: 'Mother Occ.', width: 50, key: 'mother_occupation' },
        { label: 'Mother Inc.', width: 45, key: 'mother_income' },
        { label: 'Combined Inc.', width: 50, key: 'combined_family_income' },
        { label: 'Inc. Status', width: 55, key: 'income_verification_status' },
        { label: 'Year', width: 30, key: 'application_year' },
        { label: 'Auto Elig.', width: 40, key: 'auto_eligibility' },
        { label: 'Status', width: 40, key: 'current_status' },
        { label: 'Submitted', width: 45, key: 'submitted_at' },
        { label: 'Verified', width: 45, key: 'verified_at' },
        { label: 'Remarks', width: 60, key: 'admin_remarks' },
    ];
    ...
    for (const row of rows) {
        ...
        const values = {
            ...
            father_income: row.father_income !== '' ? String(row.father_income) : '—',
            mother_income: row.mother_income !== '' ? String(row.mother_income) : '—',
            combined_family_income: row.combined_family_income !== '' ? String(row.combined_family_income) : '—',
        };
        columns.forEach((column) => {
